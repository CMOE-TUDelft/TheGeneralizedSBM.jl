function run_manufactured_solutions_transient_stokes_beta_div()

  to = TimerOutput("ManufacturedTransientStokes_beta_div")
  ...
  n_cells = (9,6)
  output_folder = datadir("sims","Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_beta_div")
  ...
  # Test parameters beta_div
  weight_approach = [:standard]
  n = [20]
  ϕ_name = [:circle]
  βdiv = [0.0,1e-3,1e-2,1e-1,1.0,1e1]
  global_gp = [false,true]
  all_params = @strdict weight_approach n ϕ_name βdiv global_gp
  ...
  # Execute beta_div case function
  function execute_case(case)
    @unpack weight_approach, n, ϕ_name, βdiv, global_gp = case
    ...
    verbose = false
    output_folder = datadir("sims","Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_beta_div",replace(case_name,".jld2"=>""))
  ...
  # Execute beta_div cases
  for case in cases
    path = datadir("sims","Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_beta_div")
  ...
  # Get data
  all_results = collect_results(datadir("sims","Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_beta_div"))
  plot_geom_cases = [:circle]
  plot_approaches = [:standard]
  plot_βdiv = [0.0,1e-3,1e-2,1e-1,1.0,1e1]
  ...
  for (igeom, geom_case) in enumerate(plot_geom_cases)
    ...
    for iplot in 1:length(plt_name)
      push!(plts,plot(xlabel="βdiv",ylabel="Error norm",legend=:best,xaxis=:log,yaxis=:log,ylims=plt_ylims[iplot]))
    ...
    for (iglobal_gp,global_gp) in enumerate(plot_global_gp)
      for (iapproach, approach) in enumerate(plot_approaches)
        results = @linq all_results[all_results.:ϕ_name.==geom_case .&& 
                                    ...
                                    all_results.:global_gp.==global_gp,:] |> orderby(:βdiv)
        for iplot in 1:length(plt_name)
          plot!(plts[iplot],results.:βdiv[2:end],results[!,plt_name[iplot]][2:end],
                marker=markers[iglobal_gp],
                color=colors[iglobal_gp],
                label="global_gp=$(global_gp)",
                xticks = (results.:βdiv[2:end], string.(results.:βdiv[2:end])))
        end
      end
    end
    for iplot in 1:length(plt_name)
      savefig(plts[iplot],plotsdir("Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_beta_div",plt_name[iplot]*"_"*string(ϕ_name[igeom])*"_bdiv.pdf"))
    end
  end
