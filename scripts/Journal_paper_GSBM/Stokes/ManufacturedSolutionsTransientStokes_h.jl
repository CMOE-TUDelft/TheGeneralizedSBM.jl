function run_manufactured_solutions_transient_stokes_h()

  to = TimerOutput("ManufacturedTransientStokes_h")
  ...
  n_cells = (9,6)
  output_folder = datadir("sims","Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_h")
  ...
  # Test parameters h convergence
  weight_approach = [:standard]
  n = [10,20,40,80]
  ϕ_name = [:circle]
  βdiv = [1.0]
  global_gp = [false,true]
  all_params_h_convergence = @strdict weight_approach n ϕ_name βdiv global_gp
  ...
  # Execute h convergence case function
  function execute_case_h_convergence(case)
    ...
    verbose = false
    output_folder = datadir("sims","Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_h",replace(case_name,".jld2"=>""))
  ...
  # Execute h convergence cases
  for case in cases
    path = datadir("sims","Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_h")
  ...
  # Get data
  all_results = collect_results(datadir("sims","Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_h"))
  ...
  for (igeom, geom_case) in enumerate(plot_geom_cases)
    ...
    for iplot in 1:length(plt_name)
      push!(plts,plot(xlabel="Number of elements per direction",ylabel=plt_label,legend=:topright,xaxis=:log,yaxis=:log))
    ...
    for (iβdiv,βdiv) in enumerate(plot_βdiv)
      for (iglobal_gp,global_gp) in enumerate(plot_global_gp)
        for (iapproach, approach) in enumerate(plot_approaches)
          ...
          for iplot in 1:length(plt_name)
            plot!(plts[iplot],results.:n,results[!,plt_name[iplot]],marker=markers[iglobal_gp],ls=lines[iβdiv],color=colors[iglobal_gp],label="global_gp=$(global_gp)")
    ...
    for iplot in 1:length(plt_name)
      savefig(plts[iplot],plotsdir("Journal_paper_GSBM","Stokes","ManufacturedTransientStokes_h",plt_name[iplot]*"_"*string(ϕ_name[igeom])*".pdf"))
    end
  end
